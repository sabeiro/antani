...
def odm2graph(trajB,isPlot=False):
    ...
    trajB.loc[:,"distance"] = trajB.apply(lambda x: haversine(x['pos_origin'][0],x['pos_origin'][1],x['pos_destination'][0],x['pos_destination'][1]),axis=1)
    G = nx.MultiDiGraph()
    G.graph['crs'] = {'init':'epsg:4326'}
    G.graph['name'] = "local network"
    for i,g in trajB.iterrows():
        G.add_edge(g['origin'],g['destination'],key=0,origin=g['origin'],destination=g['destination'],length=g['distance'],width=g['n'],speed=g['dt']/(g['n']*g['distance']))
        G.add_node(g['origin'],pos=(g['pos_origin']),x=g['pos_origin'][0],y=g['pos_origin'][1],key=0,n=g['n'])
        G.add_node(g['destination'],pos=(g['pos_destination']),x=g['pos_origin'][0],y=g['pos_origin'][1],key=0,n=g['n'])
    G = ox.project_graph(G)
    if isPlot:
