...
def removeType(G,move_type="driver"):
    ...
    tL = ['cycleway','footway','living_street','motorway','motorway_link','path','pedestrian','primary','residential','secondary','secondary_link','service','steps','tertiary','track','track_grade1','track_grade3','trunk','trunk_link','unclassified']
...
def segment2graph(geo,isPlot=False):
    ...
    for i,g in geo.iterrows():
        ...
        for start, end, j in zip(seg[:-1],seg[1:],range(len(seg))):
            ...
            G.add_edge(j1,j2,key=0,src=j1,trg=j2,speed=g['maxspeed'],highway=g['fclass'],name=g['name'],length=dist)#,geometry=line,osm_id=g['osm_id'])
...
def weightSpeed(G):
    ...
    wL = {'motorway':3,'motorway_link':3,'primary':2,'residential':0.5,'secondary':2,'secondary_link':2,'tertiary':1.5,'trunk':2,'trunk_link':2}
    keys = {}
